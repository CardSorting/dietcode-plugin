# -*- coding: utf-8 -*-
"""Project workspace resolution — never treat the plugin install tree as project root."""
...
def get_workspace_root_source() -> str:
    ...
    return raw if raw in _VALID_SOURCES else SOURCE_HERMES_PROJECT


def get_explicit_workspace_root_config() -> str:
    return str(_load_workspace_config().get("workspace_root") or "").strip()


def is_quarantined_root(path: Path | str) -> bool:
    """True when *path* is inside the plugin install root."""
    resolved = _safe_resolve(path)
    plugin = _safe_resolve(_plugin_root())
    if resolved is None or plugin is None:
        return False
    return _is_under(resolved, plugin)
...
def validate_workspace_root(
    ...
    *,
    ...
    ok = all(
        ...
        for key in ("resolved", "exists", "is_directory", "not_plugin_root", "writable")
    )
...
def resolve_workspace_root(*, source: Optional[str] = None, explicit: Optional[str] = None) -> WorkspaceResolution:
    src = source or get_workspace_root_source()
    plugin = _safe_resolve(_plugin_root())
    if explicit:
        candidate, detail = _safe_resolve(explicit), SOURCE_EXPLICIT
        detail_str = SOURCE_EXPLICIT
        candidate_path = candidate
    else:
        candidate_path, detail_str = resolve_workspace_root_path(source=src)
    validation = validate_workspace_root(candidate_path, plugin_root=plugin)
    if candidate_path is None and not validation.errors:
        validation = WorkspaceValidation(
            ok=False,
            safe_for_mutation=False,
            errors=[f"workspace_root_source={src!r} did not resolve ({detail_str})"],
            checks={"resolved": False},
        )
    return WorkspaceResolution(
        source=src,
        resolution_detail=detail_str,
        plugin_root=str(plugin) if plugin else str(_plugin_root()),
        resolved_workspace_root=str(candidate_path) if candidate_path else None,
...
def build_workspace_health() -> dict[str, Any]:
    ...
    if not report.safe_for_mutation and report.resolved_workspace_root:
        if is_quarantined_root(report.resolved_workspace_root):
            payload["hint"] = (
                "Point workspace at your Hermes project — never the plugin install tree."
            )
    elif not report.resolved_workspace_root:
        payload["hint"] = (
            "Set HERMES_KANBAN_WORKSPACE, "
            f"{_ENV_DIETCODE_WORKSPACE}, or dietcode.workspace.workspace_root."
        )
    return payload
